...
public class RedBlackTree<T extends Comparable<T>> {
	...
	private void inorderHelper(RedBlackNode<T> node) {
		if (node != nil) {
			...
			System.out.print("|" + node.key + (node.color == 1 ? "R" : "B") + "|"
					+ (node.parent != null ? node.parent.key : "NIL") + "|"
					+ (node.left != null ? node.left.key : "NIL") + "|" + (node.right != null ? node.right.key : "NIL")
					+ "|");
	...
	public void setColor(RedBlackNode<T> node) {
		if (node != nil) {
			setColor(node.left);
			if (node.key.compareTo((T) Integer.valueOf(58)) == 0)
				node.color = RedBlackNode.RED;
			else
				node.color = RedBlackNode.BLACK;
			//System.out.println("|" + (node.color == 1 ? "R" : "B") + node.key + "|"
				//	+ (node.parent != null ? node.parent.key : "NIL") + "|"
				//	+ (node.left != null ? node.left.key : "NIL") + "|" + (node.right != null ? node.right.key : "NIL")
					//+ "|");
			setColor(node.right);
...
//		tree.setC();
//
//		tree.remove(new RedBlackNode(77));
//		tree.remove(new RedBlackNode(41));
//		tree.remove(new RedBlackNode(62));
//		tree.remove(new RedBlackNode(47));
